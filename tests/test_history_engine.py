...
def test_history_mr_trends(tmp_path) -> None:
    repo = SnapshotRepository(snapshots_dir=tmp_path)
    repo.save_snapshot(Player(12), "2026-06-20")
    repo.save_snapshot(Player(15), "2026-06-21")
    engine = HistoryEngine(repo=repo)
    trends = engine.get_growth_trends()
...
def test_history_quest_activity_trends(tmp_path) -> None:
    repo = SnapshotRepository(snapshots_dir=tmp_path)
    repo.save_snapshot(Player(10, ["Quest 1"]), "2026-06-20")
    repo.save_snapshot(Player(10, ["Quest 1", "Quest 2"]), "2026-06-21")
    engine = HistoryEngine(repo=repo)
    trends = engine.get_growth_trends()
...
def test_history_relic_unlock_trends(tmp_path) -> None:
    repo = SnapshotRepository(snapshots_dir=tmp_path)
    repo.save_snapshot(Player(10, [], [], [], ["Weap1"]), "2026-06-20")
    repo.save_snapshot(Player(10, [], [], [], ["Weap1", "Weap2"]), "2026-06-21")
    engine = HistoryEngine(repo=repo)
    trends = engine.get_growth_trends()
...
def test_history_build_crafting_trends(tmp_path) -> None:
    repo = SnapshotRepository(snapshots_dir=tmp_path)
    repo.save_snapshot(Player(10, [], ["Mod1"]), "2026-06-20")
    repo.save_snapshot(Player(10, [], ["Mod1", "Mod2"]), "2026-06-21")
    engine = HistoryEngine(repo=repo)
    trends = engine.get_growth_trends()
